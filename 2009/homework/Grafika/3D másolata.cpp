...
void draw3D(OBJINFO obj, window w, PONT3D feny = PONT3D(0,0,2*s)) {
...
OBJINFO test = { kocka, kocka_lapok, 10, 16, 3, 0, 0, 0 };
...
do {
    ...
    switch ( c ) {
        case ' ':
            test.a1 += (egerx-mousex)*PI/180;
            test.a2 += (egery-mousey)*PI/180;
            mousex = egerx;
            mousey = egery;
            break;
        case 'f':
